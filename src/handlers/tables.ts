...
const ltypeHandler = (tuples: any[]) => {
  let element = null
  let offset = null
  return tuples.reduce(
    (layer: { name: any; description: any; flag: any; alignment: any; elementCount: number; patternLength: any; pattern: any[] }, tuple: any[]) => {
      const type = tuple[0]
      const value = tuple[1]
...
const layerHandler = (tuples: any[]) => {
  return tuples.reduce(
    (layer: { name: any; lineTypeName: any; colorNumber: any; flags: any; plot: boolean; lineWeightEnum: any }, tuple: any[]) => {
      const type = tuple[0]
      const value = tuple[1]
...
const styleHandler = (tuples: any[]) => {
  return tuples.reduce(
    (style: { name: any; lineTypeName: any; fixedTextHeight: any; widthFactor: any; obliqueAngle: any; flags: any; lastHeightUsed: any; primaryFontFileName: any; bigFontFileName: any }, tuple: any[]) => {
      const type = tuple[0]
      const value = tuple[1]
      switch (type) {
        case 2:
...
const vPortHandler = (tuples: any[]) => {
  return tuples.reduce(
    (vport: { name: any; handle: any; flags: any; lowerLeft: { x: number; y: number }; upperRight: { x: number; y: number }; center: { x: number; y: number }; snapSpacing: { x: number; y: number }; gridSpacing: { x: number; y: number }; direction: { x: number; y: number; z: number }; target: { x: number; y: number; z: number }; height: number; snapAngle: number; angle: number; x: number; y: number; z: number; xAxisX: number; xAxisY: number; xAxisZ: number; elevation: number }, tuple: any[]) => {
      const type = tuple[0]
      const value = tuple[1]
      switch (type) {
        case 2:
...
const tableHandler = (tuples: any[], tableType: string, handler: { (tuples: any): any; (tuples: any): any; (tuples: any): any; (tuples: any): any; (arg0: any[]): any }) => {
  const tableRowsTuples: any[][] = []
...
export default (tuples: any[]) => {
  const tableGroups: (any[] | any)[] = []
  let tableTuples
  tuples.forEach((tuple: any[]) => {
    // const type = tuple[0];
    const value = tuple[1]
