 ...
 */
const rotate = (points: any[], angle: number) => {
  return points.map(function (p: number[]) {
    return [
...
export const interpolateBSpline = (
  ...
  const controlPointsForLib = controlPoints.map(function (p: { x: any; y: any }) {
    return [p.x, p.y]
  })

  const segmentTs: any[] = [knots[degree]]
 ...
 */
export default (entity: { type: string; start: { x: any; y: any }; end: { x: any; y: any }; polyfaceMesh: any; polygonMesh: any; vertices: string | any[]; closed: any; x: any; y: any; r: any; extrusionZ: number; majorX: number; majorY: number; axisRatio: number; startAngle: any; endAngle: any; controlPoints: any; degree: any; knots: any; weights: any }, options: { interpolationsPerSplineSegment?: any }) => {
  options = options || {}
  let polyline
