 ...
 */
export const checkPinned = (k: number, knots: string | any[]) => {
  // Pinned at the start
  for (let i = 1; i < k; ++i) {
    if (knots[i] !== knots[0]) {
...
export const multiplicity = (knots: string | any[], index: number) => {
  let m = 1
 ...
 */
export const computeInsertions = (k: number, knots: string | any[]) => {
  const inserts: any[] = []
  let i = k
...
export default (k: number, controlPoints, knots) => {
  checkPinned(k, knots)
