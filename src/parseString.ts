...
const convertToTypesAndValues = (contentLines: string[]) => {
  let state = 'type'
  let type
  const typesAndValues: any[][] = []
...
const separateSections = (tuples: any[]) => {
  let sectionTuples
  return tuples.reduce((sections: any[], tuple: string[]) => {
