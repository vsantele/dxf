...
export default (parseResult: { blocks: any[]; entities: any }) => {
  const blocksByName = parseResult.blocks.reduce((acc: { [x: string]: any }, b: { name: string | number }) => {
    acc[b.name] = b
    return acc
  }, {})

  const gatherEntities = (entities: any[], transforms: any[]) => {
    let current = []
    entities.forEach((e: { type: string }) => {
      if (e.type === 'INSERT') {
        const insert = e
          ...
          for (let c = 0; c < columnCount; c++) {
            ...
            const blockEntities = block.entities.map((be) => {
              ...
              switch (be2.type) {
                ...
                case 'LWPOLYLINE':
                case 'POLYLINE': {
                  be2.vertices.forEach((v: { x: number; y: number }) => {
                    v.x -= block.x
                    v.y -= block.y
                  })
                  break
                ...
                case 'SPLINE': {
                  be2.controlPoints.forEach((cp: { x: number; y: number }) => {
                    cp.x -= block.x
                    cp.y -= block.y
                  })
                  break
